import React from "react";
import Domiduck from "../../assets/img/domiduck.svg";
import { motion } from "framer-motion";

const DashboardImageCard: React.FC<{ className?: string }> = ({ className = "" }) => (
  <motion.div
    whileHover={{ scale: 1.03 }}
    whileTap={{ scale: 0.97 }}
    className={`relative overflow-hidden rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 bg-gradient-to-br from-gray-900 to-gray-800 text-white h-48 flex ${className}`}
  >
    {/* Fondo con patrón */}
    <div className="absolute inset-0 opacity-20 bg-[url('data:image/svg+xml;base64,')]"></div>
    
    {/* Contenido ampliado */}
    <div className="relative z-10 flex items-center p-8 w-full">
      <img 
        src={Domiduck} 
        alt="Domiduck Logo" 
        className="w-20 h-20 mr-6 flex-shrink-0" 
      />
      <div>
        <h3 className="text-2xl font-bold uppercase tracking-wider mb-2">
          Domiduck
        </h3>
        <p className="text-base opacity-90">
          Innovación y calidad
        </p>
      </div>
    </div>
  </motion.div>
);

export default DashboardImageCard;